	...
	deep.View = deep.Classes(function(layer) {
		...
		load: function(opt) {
			...
			if (config.scope != "browser")
				return;
			var self = this;
			if (this.init && (deep.Promise.context.concurrency || !this.initialised))
		...
		refresh: function(arg /* params, loaded, routeMatch */ ) {
			...
			var render = function(loaded) {
				...
				if (placed && !deep.Promise.context.concurrency)
				...
				if (promise)
					return promise
						.done(function() {
							if(config.relink !== false && deep.isBrowser && placed && self.relink)
								self.relink(placed);
