...
define(["require", "deepjs/deep", "deepjs/lib/errors", "./directives"], function(require, deep, errors) {
"use strict";
	...
	deep.errors.View = function(msg, report, fileName, lineNum){
	...
	deep.View = deep.Classes(function(layer) {
		...
		load: function(opt) {
			...
			if (config.scope != "browser")
				return;
			var self = this;
			if (this.init && (deep.context("isServer") || !this.initialised))
			...
			if (this.condition === false)
				return null;
			var promises = [], ctx = { params:params, context:deep.context(), ctrl:self };
			if (this.condition)
				if (typeof this.condition === "function" && !this.condition.apply(ctx))
					return null;
			if (this.what)
				promises.push(deep.nodes(this).query("./what").deepLoad(ctx, false));
			else
				promises.push(null);

			if (typeof this.how === 'string')
				promises.push(deep.nodes(this).query("./how").deepLoad(ctx, false, true));
			else
				promises.push(this.how);

			if (this.where)
				if (typeof this.where === 'string')
					promises.push(deep.nodes(this).query("./where").deepLoad(ctx, false, true));
				else
		...
		refresh: function(arg /* params, loaded, routeMatch */ ) {
			...
			if (params && !deep.context("isServer"))
				this.params = params;
			...
			var render = function(loaded) {
				...
				if (placed && !deep.context("isServer"))
				...
				if (promise)
					return promise
						.done(function() {
							if(config.relink !== false && !deep.context("isServer") && placed && self.relink)
								self.relink(placed);
			...
			.each(function() {
				...
				$(this).click(function(e) {
					if(e && e.preventDefault)
						e.preventDefault();
					deep.app(app).when(app.views.get(uri)).elog();
					return false;
				});
			});
		}
